...
const (
	...
	bcm2712FrontButtonPin = 20
	bcm2712StealthPin     = 21
	bcm2712RegPwmTachPin  = 13
	...
	bcm2712RegPwmCtlBitUsef1 = 5 // Use FIFO
	bcm2712RegPwmCtlBitSbit1 = 3 // Line level when not transmitting
	bcm2712RegPwmCtlBitRptl1 = 2 // Repeat last data when FIFO is empty
	bcm2712RegPwmCtlBitMode1 = 1 // Mode; 0: PWM, 1: Serializer
	bcm2712RegPwmCtlBitPwen1 = 0 // Enable (pwm1)

	bcm2712RegPwmclkCntrl          = 0x28
	bcm2712RegPwmclkDiv            = 0x29
	bcm2712RegPwmclkCntrlBitSrcOsc = 0
	bcm2712RegPwmclkCntrlBitEnable = 4
...
func NewCm5Hal(ctx context.Context, opts ComputeBladeHalOpts) (ComputeBladeHal, error) {
	...
	gpioMem, gpioMem8, err := mmap(devmem, bcm2712GpioAddr, bcm2712PageSize)
	if err != nil {
		return nil, err
	}
	pwmMem, pwmMem8, err := mmap(devmem, bcm2712RegPwmAddr, bcm2712PageSize)
	if err != nil {
		return nil, err
	}
	clkMem, clkMem8, err := mmap(devmem, bcm2712ClkAddr, bcm2712PageSize)
	if err != nil {
		return nil, err
	}
...
func (bcm *bcm2712) setPwm0Freq(targetFrequency uint64) error {
	...
	bcm.pwmMem[bcm2712RegPwmCtl] &^= (1 << bcm2712RegPwmCtlBitPwen1) | (1 << bcm2712RegPwmCtlBitPwen2)
	time.Sleep(time.Microsecond * 10)

	// Stop clock w/o any changes, they cannot be made in the same step
	bcm.clkMem[bcm2712RegPwmclkCntrl] = bcm2712ClkManagerPwd | (bcm.clkMem[bcm2712RegPwmclkCntrl] &^ (1 << 4))
	time.Sleep(time.Microsecond * 10)

	// Wait for the clock to not be busy so we can perform the changes
	for bcm.clkMem[bcm2712RegPwmclkCntrl]&(1<<7) != 0 {
		time.Sleep(time.Microsecond * 10)
	}

	// passwd, disabled, source (oscillator)
	bcm.clkMem[bcm2712RegPwmclkCntrl] = bcm2712ClkManagerPwd | (0 << bcm2712RegPwmclkCntrlBitEnable) | (1 << bcm2712RegPwmclkCntrlBitSrcOsc)
	time.Sleep(time.Microsecond * 10)

	bcm.clkMem[bcm2712RegPwmclkDiv] = bcm2712ClkManagerPwd | (uint32(divisor) << 12)
	time.Sleep(time.Microsecond * 10)

	// Start clock (passwd, enable, source)
	bcm.clkMem[bcm2712RegPwmclkCntrl] = bcm2712ClkManagerPwd | (1 << bcm2712RegPwmclkCntrlBitEnable) | (1 << bcm2712RegPwmclkCntrlBitSrcOsc)
	time.Sleep(time.Microsecond * 10)

	// Start pwm for both channels again
	bcm.pwmMem[bcm2712RegPwmCtl] &= (1 << bcm2712RegPwmCtlBitPwen1)
	time.Sleep(time.Microsecond * 10)
...
func (bcm *bcm2712) setFanSpeedPWM(speed uint8) {
	...
	// Use fifo, repeat, ...
	bcm.pwmMem[bcm2712RegPwmCtl] = (1 << bcm2712RegPwmCtlBitPwen1) | (1 << bcm2712RegPwmCtlBitMode1) | (1 << bcm2712RegPwmCtlBitRptl1) | (1 << bcm2712RegPwmCtlBitUsef1)
	time.Sleep(10 * time.Microsecond)
	bcm.pwmMem[bcm2712RegPwmRng1] = 32
	time.Sleep(10 * time.Microsecond)
	bcm.pwmMem[bcm2712RegPwmFif1] = targetvalue
...
func (bcm *bcm2712) updateLEDs() error {
	...
	// -> bcm2712RegGpfsel1 24:26, regular output; it's configured as alt5 whenever pixel data is sent.
	// This is not optimal but required as the pwm0 peripheral is shared between fan and data line for the LEDs.
	time.Sleep(10 * time.Microsecond)
	bcm.gpioMem[bcm2712RegGpfsel1] = (bcm.gpioMem[bcm2712RegGpfsel1] &^ (0b111 << 24)) | (0b010 << 24)
	time.Sleep(10 * time.Microsecond)
	defer func() {
		// Set to regular output again so the PWM signal doesn't confuse the WS2812
		bcm.gpioMem[bcm2712RegGpfsel1] = (bcm.gpioMem[bcm2712RegGpfsel1] &^ (0b111 << 24)) | (0b001 << 24)
		bcm.setFanSpeedPWM(bcm.currFanSpeed)
	}()

	bcm.pwmMem[bcm2712RegPwmCtl] = (1 << bcm2712RegPwmCtlBitMode1) | (1 << bcm2712RegPwmCtlBitRptl1) | (0 << bcm2712RegPwmCtlBitSbit1) | (1 << bcm2712RegPwmCtlBitUsef1) | (1 << bcm2712RegPwmCtlBitClrf1)
	time.Sleep(10 * time.Microsecond)
	// bcm.pwmMem[bcm2712RegPwmRng1] = 32
	bcm.pwmMem[bcm2712RegPwmRng1] = 24 // we only need 24 bits per LED
	time.Sleep(10 * time.Microsecond)
	...
	bcm.pwmMem[bcm2712RegPwmFif1] = 0
	bcm.pwmMem[bcm2712RegPwmFif1] = 0
	bcm.pwmMem[bcm2712RegPwmFif1] = 0
	bcm.pwmMem[bcm2712RegPwmFif1] = 0
	bcm.pwmMem[bcm2712RegPwmFif1] = 0
	bcm.pwmMem[bcm2712RegPwmFif1] = 0
	// Write top LED data
	bcm.pwmMem[bcm2712RegPwmFif1] = serializePwmDataFrame(bcm.leds[0].Red) << 8
	bcm.pwmMem[bcm2712RegPwmFif1] = serializePwmDataFrame(bcm.leds[0].Green) << 8
	bcm.pwmMem[bcm2712RegPwmFif1] = serializePwmDataFrame(bcm.leds[0].Blue) << 8
	// Write edge LED data
	bcm.pwmMem[bcm2712RegPwmFif1] = serializePwmDataFrame(bcm.leds[1].Red) << 8
	bcm.pwmMem[bcm2712RegPwmFif1] = serializePwmDataFrame(bcm.leds[1].Green) << 8
	bcm.pwmMem[bcm2712RegPwmFif1] = serializePwmDataFrame(bcm.leds[1].Blue) << 8
	// make sure there's >50us of silence
	bcm.pwmMem[bcm2712RegPwmFif1] = 0 // auto-repeated, so no need to feed the FIFO further.

	bcm.pwmMem[bcm2712RegPwmCtl] = (1 << bcm2712RegPwmCtlBitPwen1) | (1 << bcm2712RegPwmCtlBitMode1) | (1 << bcm2712RegPwmCtlBitRptl1) | (0 << bcm2712RegPwmCtlBitSbit1) | (1 << bcm2712RegPwmCtlBitUsef1)
